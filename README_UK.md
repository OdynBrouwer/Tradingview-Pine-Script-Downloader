...
```bash
# For containerized environments (e.g., Ubuntu 24)
...
### Issue 1: SyntaxWarning (Line ~1179)
...
**Location**: `_normalize_source()` method

**Current code**:
```python
source = source.replace('\\n', '\n').replace('\\r', '\r').replace('\\t', '\t')
source = source.replace('\"', '"').replace('\/', '/')  # ⚠️ Invalid escape
```
...
# Option 2: Escape the backslash
source = source.replace('\"', '"').replace('\\/', '/')
```

### Issue 2: Google Login Modal Blocking Source Tab

**Symptoms**: "Inloggen met Google" overlay prevents clicking Source tab

**Status**: Fixed in latest version (added to `handle_overlays()`)

**Fix details**:
```python
# Now searches for Dutch/English Google login copy and hides modals
# Also blocks OAuth iframes by URL pattern
```

### Issue 3: Viewport Size Inconsistency

**Symptoms**: Copy button position varies between batch and single-script runs

**Status**: Fixed - now uses consistent 1280×720 viewport

**Previous issue**:
- Batch mode used multiple viewport sizes
- Context restarts reverted to different dimensions

**Current solution**:
```python
# All contexts use 1280×720 (16:9 aspect ratio)
viewport={'width': 1280, 'height': 720}
```

### Issue 4: Import-Only Snippet Captures

**Symptoms**: Diagnostic captures contain `import user/library/1` but not full source

**Status**: Fixed - import-only captures filtered out
