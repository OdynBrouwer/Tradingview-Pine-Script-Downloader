...
r = Path('analyze/jsons/failed-pubdates-retry3.json')
if not r.exists():
    raise SystemExit('retry3 file missing')
...
for cat in cats:
    ...
    for it in rarr:
        ...
        if u in m:
            ...
            # If URL not present, add it
            m[u] = it
            updated += 1
    new = list(m.values())
    f.write_text(json.dumps(new, indent=2, ensure_ascii=False), encoding='utf-8')
print('merged retry3 results into category files; updated/added entries:', updated)
